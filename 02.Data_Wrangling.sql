...
/*-showing the unique number of department*/
...
/*-showing the unique   department with uppercase*/
SELECT DISTINCT(UPPER(department))
FROM staff
ORDER BY 1;


/*-showing the unique department with lowercase*/
...
/*** Concatetation ***/
/*-last name of employee storing in another column */
SELECT 
	last_name,
...
/*selecting the job tittle where starting name is assistant*/
SELECT job_title
FROM staff
WHERE job_title LIKE 'Assistant%';
...
SELECT
	job_title
FROM staff
WHERE job_title like '%Assistant%(III|IV)';
